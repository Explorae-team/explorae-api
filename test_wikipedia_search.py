...
attractions = [
    ...
    'Casa do Artista Popular', 'Museu de Arte Sacra da Paraíba', 'Igreja de Santa Terezinha', 
...
def search_wikipedia_image(query):
    # Dicionário de mapeamentos específicos para otimizar a busca
    custom_mapping = {
    ...
    # 1. Tentar primeiro busca exata de título
    try:
        params = {
            "action": "query",
            "format": "json",
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            for page_id, page in pages.items():
                if page_id != "-1" and "original" in page:
                    return page["original"].get("source"), "exact_title"
    except Exception:
        pass
        
    # 2. Tentar busca textual se a busca por título exato falhar
    try:
        # Adiciona contexto para evitar resultados ambíguos de outros estados/países
        query_search = search_term
        if "João Pessoa" not in query_search and "Paraíba" not in query_search and "Cabedelo" not in query_search and "Conde" not in query_search and "Lucena" not in query_search:
            query_search += " João Pessoa Paraíba"
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            for page_id, page in pages.items():
                if page_id != "-1" and "original" in page:
                    return page["original"].get("source"), "search_fallback"
    except Exception:
        pass
        
    return None, "not_found"

print("Iniciando varredura de imagens da Wikipedia para as 105 atrações...")
results = {}
for i, name in enumerate(attractions):
    img, method = search_wikipedia_image(name)
    results[name] = {"image": img, "method": method}
    print(f"[{i+1}/105] {name}: {method} -> {img}")
    time.sleep(0.1)

# Escreve os resultados em um JSON para analisarmos
with open("wikipedia_images_results.json", "w", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False, indent=4)
print("Concluído! Salvo em 'wikipedia_images_results.json'")
