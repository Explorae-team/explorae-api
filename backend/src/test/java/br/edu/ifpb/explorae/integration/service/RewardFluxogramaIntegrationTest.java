...
public class RewardFluxogramaIntegrationTest {
    ...
    // =========================================================================
    // FASE 2: GRAFO DE CONTROLE DE FLUXO DA VALIDAÇÃO DE VOUCHER (Token JWT)
    // SUT: RewardService.validateVoucherToken(String token)
    // Complexidade Ciclomática: V(G) = 6 Caminhos Independentes
    // =========================================================================

    @Autowired
    private br.edu.ifpb.explorae.user.service.TokenService tokenService;

    @Test
    @DisplayName("CB-V1: Token JWT Inválido/Corrompido -> BusinessException")
    public void testCB_V1_TokenInvalidoOuCorrompido() {
        String tokenMalicioso = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.token.invalido";

        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> rewardService.validateVoucherToken(tokenMalicioso),
                "Deveria rejeitar token JWT forjado ou corrompido."
        );

        assertEquals("QR Code inválido, corrompido ou expirado.", exception.getMessage());
    }

    @Test
    @DisplayName("CB-V2: Token JWT Válido mas Voucher Fantasma (Inexistente no Banco) -> ResourceNotFoundException")
    public void testCB_V2_VoucherNaoEncontradoNoBanco() {
        UUID voucherFantasmaId = UUID.randomUUID();
        String tokenValidoSemRegistro = tokenService.generateVoucherToken(voucherFantasmaId);

        ResourceNotFoundException exception = assertThrows(
                ResourceNotFoundException.class,
                () -> rewardService.validateVoucherToken(tokenValidoSemRegistro),
                "Deveria falhar ao consultar banco para voucher inexistente."
        );

        assertEquals("Voucher não encontrado no sistema.", exception.getMessage());
    }

    @Test
    @DisplayName("CB-V3: Bloqueio de Fraude de Gasto Duplo (Status == USED) -> BusinessException")
    public void testCB_V3_VoucherJaUtilizado_GastoDuplo() {
        ...
        Voucher voucherUsado = voucherRepository.save(Voucher.builder()
                .user(savedUser)
                .reward(reward)
                .code("EXP-JAUSADO")
                .status(VoucherStatus.USED)
                .expiresAt(java.time.LocalDateTime.now().plusDays(10))
                .usedAt(java.time.LocalDateTime.now().minusHours(1))
                .build());

        String token = tokenService.generateVoucherToken(voucherUsado.getId());

        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> rewardService.validateVoucherToken(token),
                "Deveria barrar voucher já utilizado."
        );

        assertEquals("Este voucher já foi utilizado.", exception.getMessage());
    }

    @Test
    @DisplayName("CB-V4: Voucher Inativo ou Cancelado -> BusinessException")
    public void testCB_V4_VoucherStatusInativo() {
        Reward reward = rewardRepository.save(Reward.builder()
                .partner(savedPartner)
                .name("Suco")
                .description("Item")
        ...
        // Status diferente de ACTIVE e de USED (ex: EXPIRED prévio)
        Voucher voucherExpiradoPrevio = voucherRepository.save(Voucher.builder()
                .user(savedUser)
                .reward(reward)
                .code("EXP-EXPPREV")
                .status(VoucherStatus.EXPIRED)
                .expiresAt(java.time.LocalDateTime.now().minusDays(2))
                .build());

        String token = tokenService.generateVoucherToken(voucherExpiradoPrevio.getId());

        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> rewardService.validateVoucherToken(token),
                "Deveria barrar voucher inativo."
        );

        assertEquals("Este voucher está inativo ou expirado.", exception.getMessage());
    }

    @Test
    @DisplayName("CB-V5: Voucher Ativo porém com Data Vencida -> Atualiza para EXPIRED no Banco e Rejeita")
    public void testCB_V5_VoucherAtivoPoremExpiradoTemporalmente() {
        Reward reward = rewardRepository.save(Reward.builder()
                .partner(savedPartner)
                .name("Almoço")
                .description("Item")
        ...
        Voucher voucherVencido = voucherRepository.save(Voucher.builder()
                .user(savedUser)
                .reward(reward)
                .code("EXP-TEMPVENC")
                .status(VoucherStatus.ACTIVE) // Status ainda ativo
                .expiresAt(java.time.LocalDateTime.now().minusDays(1)) // Mas a data venceu ontem
                .build());

        String token = tokenService.generateVoucherToken(voucherVencido.getId());

        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> rewardService.validateVoucherToken(token),
                "Deveria rejeitar validação de voucher vencido."
        );

        assertEquals("Este voucher expirou e não pode ser validado.", exception.getMessage());

        // Verificação de Integração no Banco: O sistema deve ter atualizado o status para EXPIRED
        Voucher voucherAtualizado = voucherRepository.findById(voucherVencido.getId()).orElseThrow();
        assertEquals(VoucherStatus.EXPIRED, voucherAtualizado.getStatus(), "O voucher deve ter transitado para EXPIRED no banco.");
    }

    @Test
    @DisplayName("CB-V6: Caminho Feliz de Validação no Balcão: Transita para USED, grava timestamp e comita")
    public void testCB_V6_CaminhoFeliz_ValidacaoEConsumoNoBalcao() {
        Reward reward = rewardRepository.save(Reward.builder()
                .partner(savedPartner)
        ...
        Voucher voucherAtivo = voucherRepository.save(Voucher.builder()
                .user(savedUser)
                .reward(reward)
                .code("EXP-VALSUCESSO")
        ...
        String token = tokenService.generateVoucherToken(voucherAtivo.getId());

        br.edu.ifpb.explorae.gamification.dto.VoucherValidationResponseDTO response =
                rewardService.validateVoucherToken(token);

        // 1. Asserções do DTO de resposta do lojista
        assertNotNull(response, "Resposta não pode ser nula.");
        assertEquals("EXP-VALSUCESSO", response.voucherCode());
        assertEquals("Petisco Especial", response.rewardName());
        assertEquals(savedPartner.getName(), response.partnerName());
        assertEquals(savedUser.getName(), response.userName());
        assertNotNull(response.validatedAt(), "Timestamp de validação deve ser preenchido.");

        // 2. Asserções de Persistência no Banco de Dados
        Voucher voucherConsumido = voucherRepository.findById(voucherAtivo.getId()).orElseThrow();
        assertEquals(VoucherStatus.USED, voucherConsumido.getStatus(), "Voucher deve estar com status USED no banco.");
        assertNotNull(voucherConsumido.getUsedAt(), "Data de uso deve estar gravada no banco.");
    }
}
